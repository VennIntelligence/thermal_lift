...
def _write_baseline_config(metrics: dict[str, Any], output_path: Path) -> None:
    source_dir = Path(str(metrics.get("output_dir", "")))
    metrics_path = source_dir / "metrics.json" if str(source_dir) else None
    if metrics_path is not None:
        try:
            metrics_path_text = str(metrics_path.relative_to(PROJECT_ROOT))
        except ValueError:
            metrics_path_text = str(metrics_path)
    else:
        metrics_path_text = None
    payload = {
        "source": {
            "episode": "EP06",
            "method": "MAP-TV",
            "status": "generated_ep08_stage3_248_clean_fullframe",
            "note": (
                "Generated by scripts/generate_ep06_stage3_baseline.py from the EP08 Stage 3 "
                "real-data protocol. This replaces the legacy 255-frame EP06 proxy."
            ),
        },
        "map_tv": {
            "holdout_residual": metrics.get("holdout_residual"),
            "split_half_nrmse": metrics.get("split_half_nrmse"),
            "artifact_score": metrics.get("artifact_score"),
            "raw_control_agreement": metrics.get("raw_control_agreement"),
            "p95_gradient": metrics.get("p95_gradient"),
            "n_frames": metrics.get("n_frames"),
            "train_frame_count": metrics.get("train_frame_count"),
            "val_frame_count": metrics.get("val_frame_count"),
            "metric_source_file": metrics_path_text,
            "metric_source_protocol": metrics.get("protocol"),
            "metric_source_shape": "x".join(str(v) for v in metrics.get("hr_shape", [])),
        },
        "gate_thresholds": {
            "holdout_residual_max_ratio_to_map_tv": 1.0,
            "split_half_nrmse_max_ratio_to_map_tv": 1.0,
            "artifact_score_max_ratio_to_map_tv": 1.0,
            "raw_control_agreement_rule": "higher_is_better_relative_to_map_tv",
            "pin_region_visual_rule": None,
            "note": (
                "Ratios are anchors for comparison, not automatic proof of SR success; "
                "visual contour evidence and artifact checks remain required."
            ),
        },
    }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Also update configs/ep06_baseline_metrics.json from this generated run.",
    )
    parser.add_argument("--baseline-config-output", type=Path, default=BASELINE_CONFIG)
    return parser
...
def run_map_tv_stage3_baseline(args: argparse.Namespace) -> None:
    ...
    if bool(getattr(args, "write_baseline_config", False)):
        _write_baseline_config(metrics, Path(args.baseline_config_output))
        print(f"updated EP06 baseline config at {Path(args.baseline_config_output)}")
