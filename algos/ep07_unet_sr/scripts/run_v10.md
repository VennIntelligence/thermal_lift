...
CUDA_VISIBLE_DEVICES=0 uv run python -m unet_sr.train \
  ...
  --output-dir outputs/ep07_v10_smoke_lam001 \
  --input-mode hybrid_drizzle2x \
  --residual-mode drizzle2x \
...
| Scalar | 期望 |
|---|---|
| `loss/residual_penalty` | 有限，量级可用于标定 lambda |
| `residual/delta_mean` | 不应快速发散到大 DC offset |
| `residual/delta_std` | 应随结构学习增长但保持有限 |

lambda 初始目标：惩罚项 `lambda * loss/residual_penalty` 在训练初期占 `loss/total` 的 10-30%。若 smoke 中占比明显过低或过高，先重标定下面三档。

## Full Lambda Sweep

默认三档先用 `0.003 / 0.01 / 0.03`。这不是物理常数；若 smoke 的 loss 量级显示不合适，按上面的 10-30% 原则整体缩放。
...
CUDA_VISIBLE_DEVICES=0 uv run python -m unet_sr.train \
  ...
  --output-dir outputs/ep07_v10_residual_lam0003 \
  --input-mode hybrid_drizzle2x \
  --residual-mode drizzle2x \
  --residual-penalty-weight 0.003 \
  --scale 2 \
  --batch-size 128 \
  --num-workers 8 \
  --total-steps 25000 \
...
CUDA_VISIBLE_DEVICES=0 uv run python -m unet_sr.train \
  --training-pool-dir ../../data/synthetic/training_pool_2x_aa_burst \
  --output-dir outputs/ep07_v10_residual_lam001 \
  --input-mode hybrid_drizzle2x \
  --residual-mode drizzle2x \
  --residual-penalty-weight 0.01 \
  --scale 2 \
...
CUDA_VISIBLE_DEVICES=1 uv run python -m unet_sr.train \
  --training-pool-dir ../../data/synthetic/training_pool_2x_aa_burst \
  --output-dir outputs/ep07_v10_residual_lam003 \
  --input-mode hybrid_drizzle2x \
  --residual-mode drizzle2x \
  --residual-penalty-weight 0.03 \
  --scale 2 \
  --batch-size 128 \
  --num-workers 8 \
  --total-steps 25000 \
...
uv run python scripts/v9_review/run_pareto_sweep.py \
  --run-dir outputs/ep07_v10_residual_lam001 \
