...
def _make_scene(root: Path) -> Path:
    ...
    coverage = np.ones((hr_h, hr_w), dtype=np.float32)
    obs_4x = np.stack(
        [
            target_like + 0.01 * np.random.default_rng(0).normal(size=(hr_h, hr_w)).astype(np.float32),
            coverage,
            np.full((hr_h, hr_w), 0.01, dtype=np.float32),
        ],
        axis=0,
    )
    ...
    np.savez_compressed(scene / "obs_features_4x.npz", obs_features=obs_4x)
