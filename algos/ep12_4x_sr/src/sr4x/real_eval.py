"""Real-data evaluation for TensorBoard during EP12 4x SR training."""
...
def _temperature_rgb(
    ...
    *,
    ...
    """Temperature panel for TensorBoard (inferno, 1–99 percentile).
...
def save_temperature_figure(
    ...
    *,
    ...
    """Save center-zoom temperature figure to disk.
    ...
    ax.set_title(f"UNet {scale}x @ EP12 step {step} (temperature)")
    ax.set_xticks([])
    ax.set_yticks([])
...
def artifact_score(sr_img: np.ndarray, *, scale: int = 4) -> float:
...
def maybe_log_real_eval(
    ...
    *,
    ...
    """Run real-data inference and log center-zoom panels to TensorBoard."""
    ...
    drizzle_scale = int(training_config.drizzle_scale)
    patch_size = int(training_config.patch_size)
    sigma_bg = float(config.highpass_sigma)
    ...
    if baseline_hp is not None and baseline_hp.shape != unet_hp.shape:
        zoom_factor = unet_hp.shape[0] / baseline_hp.shape[0]
        baseline_hp = ndimage.zoom(baseline_hp, zoom=zoom_factor, order=1)
    ...
    if config.save_png and config.output_dir:
        ...
        temp_png = save_temperature_figure(
            unet_temp,
        ...
        print(f"Saved temperature figure: {temp_png}")
