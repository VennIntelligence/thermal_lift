...
> 2. **判据修订**：§3 的"real split-half FRC 为主 gate"对神经方法**不可用**——确定性网络在两半复现同一先验高频，self-FRC≈1 是幻觉签名不是分辨率。改用 **cross-method FRC**（`--cross-pair`）+ 同半幅对照 + M2 负对照。0d 套件仅 extent 探针有好/坏判别力。
