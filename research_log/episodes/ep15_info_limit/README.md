# EP15: 4x 信息上限实测 + 经典去卷积基准

> **⚠️ 数字口径警示（2026-07-24 收尾标注）**: 本 README 的 M2 FRC cutoff 17.03 µm 使用**旧标定单位（10 µm/pixel 时代）**，按 pitch 重标定（ACL-023，20 µm/px）换算即 34.07 µm——这正是旧对齐时代的权威值。ACL-048 精修对齐后，权威可恢复频带已修正为 **25.45 ± 0.73 µm**。本 README 的定量数字未回填，仅保留为历史记录；引用请以 `algorithm_changelog.md` 顶部速览与 ACL-048 为准。M1 相位占用结论（detector-axis 11/25 → 2x 为合理倍率）不受此影响，仍然有效。

**定位**: EP12 4x 网络失败后，用第一性原理测量数据真实信息上限（FRC），并用经典 MAP-TV 去卷积建立后续网络方法必须超越的下限基准。
...
- `contour_refined` 在 detector-axis 5x bin 上塌缩到 11/25，应作为 4x/5x 高倍率风险信号；M2 FRC、M3 σ 仲裁和 M4 去卷积基准仍然必须执行。
...
后续判据：任何 4x UNet/Transformer 若不能在 **FRC 频带一致性** 和 **zigzag FWHM / dip 指标** 上同时优于这个 MAP-TV 基准，则不予采纳。负结果也同样有效：若网络只产生更锐视觉但 FRC 或 zigzag 剖面不优于 M4，应视为伪锐化风险。
