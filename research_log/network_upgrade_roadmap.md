...
> - **Step 4 的"synthetic split-half FRC 快 sanity"照旧，但真实域主 gate 改为 cross-method FRC**（self split-half FRC 对神经方法结构性失效，奖励可复现幻觉），且神经×经典比较**必须先做偏移校正**——solver 输出网格带 +0.5 HR px 角点约定（ACL-049，`forward_torch.py` 设计使然，不改训练约定、修在对比层：`probe_pair_offset.py` / `real_eval.to_center_grid`）。
> - **神经臂现状（ACL-049 v4）**：校正后神经与经典同档（V11 cutoff 平 TGV 23.03µm，@30µm 差 0.05-0.06）；"神经破坏带内信息"的历史结论全部作废。当前基线=TGV×drz 0.702@30µm / 23.03µm，Step 4 及格线据此执行。
>
