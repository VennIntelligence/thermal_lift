# %% [markdown]
# ## 5. Evidence-Use Table / 证据使用边界表
#
# EP02 的最终产物不是一个替代 theta 或替代 detector pitch 的新配置，而是一张证据分工表：哪些信息用于 prior，哪些用于 alignment truth，哪些 pair 只能用于诊断。
#
...
# |---|---|---|---|---|---|---|
# | stage/文件名坐标 prior | Stage/filename coordinate prior | 覆盖规划、初始化、正则约束 | coverage planning, initialization, regularization | alignment truth 或成功指标 | alignment truth or success metric | commanded X/Y 是元数据；真实帧间对齐必须由图像数据检查 |
...
# > **怎么读表**: 先看“allowed/use”列，确认某类证据能支持什么；再看“forbidden/use”或原因列，确认它不能支持什么。尤其要区分 command prior、局部 NCC/ESF 响应和最终 alignment truth。
...
# - 当前可用的 alignment score 显示 data-driven contour/NCC 对齐比 filename/stage prior 更适合作为对齐真值和质量门控。
#
