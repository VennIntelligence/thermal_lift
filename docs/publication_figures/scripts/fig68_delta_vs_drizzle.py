...
fig, axes = plt.subplots(2, 4, figsize=(W_DOUBLE * 0.72, 3.15))
fig.set_layout_engine("none")

axes[0, 0].imshow(drz, cmap="inferno", vmin=-vt, vmax=vt,
                  interpolation="nearest")
axes[0, 0].set_title("Drizzle (anchor)", fontsize=8)
axes[1, 0].axis("off")
axes[1, 0].annotate(
    "difference maps:\nred = warmer than\ndrizzle, blue = cooler.\n"
    "Fringes hugging trace\nedges = sharpening\nanchored to real\nstructure.",
    (0.02, 0.5), xycoords="axes fraction", ha="left", va="center",
...
for j, (name, _) in enumerate(ARMS, start=1):
    ...
    axes[1, j].annotate(f"rms {np.std(diffs[name]):.3f}",
                        ...
                        va="bottom", fontsize=6, color="#222222",
...
axes[1, 1].set_ylabel("arm $-$ drizzle", fontsize=8)

cax = fig.add_axes([0.915, 0.10, 0.012, 0.36])
cb = fig.colorbar(im_d, cax=cax)
cb.set_label("$\\Delta$ [$^\\circ$C]", fontsize=6.5)
...
fig.suptitle("Where each arm departs from the drizzle anchor",
             x=0.03, y=0.985, ha="left", fontsize=9)
fig.subplots_adjust(left=0.02, right=0.905, top=0.855, bottom=0.045,
