...
> 数字红线：**TB-scale 与 EP11-harness scale 绝不混在同一图/表**（见 `zh/07_experiments.zh.md` 头注）。

## 主文图（目标 5 图 + 2 表，8 页版式）

### F0 — Teaser（§1 页首，单栏）

- **内容**: F5 的中心 zigzag ROI 三列裁剪：bicubic / TGV / 最优学习方法（温度域）
...
### F3 — null-space drift 轨迹 + forward-loss inset（§5.2，双栏全宽；全文核心机制图）

- **内容**: 五个 1x 输入变体（HotLoss / Stats / Stats+PixelShuffle / Stats+HP-FC / Stats+Full-FC）的 artifact & corr vs step 双 panel；
  inset：`loss/forward_model` 贴底曲线（HotLoss / Stats+HP-FC / Stats+Full-FC，log y）；canonical ○ / 60K 端点 ×
- **状态**: ✅ 当前稿已生成（Task B）→ `algos/ep07_unet_sr/scripts/plot_drift_trajectories_paper.py`；
  Stats+Full-FC / Hybrid / Hybrid+Native-FC 训练已完成，当前稿可按需 `--refresh`
...
- **资产**: 轨迹主体 `output/ep11_dl_benchmark/checkpoint_selection/checkpoint_metrics.csv`
  （HotLoss/Stats/Stats+PixelShuffle/Stats+HP-FC）+ `fig_trajectories.png`（旧版无 inset）；forward loss 在各 run
  `tb_logs/` 的 tag `loss/forward_model`
- **红线**: Hybrid（run id: v9a）不进主图（proxy 跨输入模式不可横比），出 supp companion 图

### F4 — proxy Pareto + checkpoint 选择（附录 C.5，单栏；主文 §5.1 表注引用）

- **内容**: 四个变体的 Pareto 散点 + TGV 参考点 (0.695, 0.916)（TB-scale）+ canonical 标注
- **状态**: ✅ `output/ep11_dl_benchmark/checkpoint_selection/fig_pareto.png` +
  `checkpoint_candidates.csv`；Hybrid / Hybrid+Native-FC / Stats+Full-FC / Hybrid+ResObs 终稿数字以统一 harness 表为准，F4 可按最终文案可选 refresh
- **视觉 gate 配套**: `panel_v6/v8.1a/v8.1b/v9b.png`（文件名保留 run id；panel 类材料放 supp）

### F5 — 主视觉对比（§5.1，双栏全宽，含 F6 消融行）

- **内容**: 行 1 温度域 / 行 2 highpass 域 × 列 {drizzle, TGV, Hybrid late endpoint 60K,
  Hybrid+ResObs λ=1.2@15K}；中心 zigzag ROI 用来显示 drizzle softness、TGV staircase、Hybrid late endpoint
  over-thickening、Hybrid+ResObs sharp/grain trade-off。
...
- **列**: split-half NRMSE / artifact / raw-control corr / FRC@{16,14,12} µm /
  zigzag median FWHM & dip / runtime；**行**: bicubic / drizzle / MAP-TV / TGV / UNet-best / Hybrid-best
- **状态**: ✅ 统一 harness 完成（Task D）
- **终稿数据**: `output/ep11_unified_harness/t1_metrics.csv`；
  全方法扩展：`output/ep11_unified_harness/all_arm_metrics.csv`；
...
### T2 — 消融矩阵 input × anchor（§5.3）

- **格**: {1x stats, hybrid drizzle} × {none, HP-FC, Full-FC, Native-FC}；
  已填 1x×none(Stats)、1x×HP-FC(Stats+HP-FC)、1x×Full-FC(Stats+Full-FC)、hybrid×none(Hybrid)、
  hybrid×Native-FC(Hybrid+Native-FC)，另列 Hybrid+ResObs residual-over-observation 作为参数化输出对照
- **状态**: ✅ 统一 harness 完成（Task D）
- **数据源**: `output/ep11_unified_harness/t2_metrics.csv` + 各变体 `config.json` 字段
...
| S-F3 | Hybrid 轨迹 companion 图（run id: v9a） | `output/paper_figures/fig03s_v9a_trajectory.*` | ✅ 当前稿，训练后 refresh |
| S-F4 | 四个变体 checkpoint 视觉 gate panel | `checkpoint_selection/panel_*.png` | ✅ |
| S-F5 | 各变体 step 序列视觉演化（漂移可视化） | `algos/ep07_unet_sr/outputs/*/eval_real/` | ✅ 选图即可 |
...
| S-F8 | E3 对齐源消融 + F7 全曲线 | `output/ep16_budget_robustness/` | ✅ 经典方法完成 |
| S-F9 | 零训练融合 baseline Pareto 叠加（Hybrid 轨迹 + 4 条融合曲线 + 支配象限 + TGV/drizzle 参考点） | `output/paper_figures/figS09_fusion_pareto.{png,pdf}`（`scripts/paper_figures/figS09_fusion_pareto.py`；源 `output/ep07_v9_review/*.csv`） | ✅ CVPR 风格（06-12）；Hybrid+ResObs 落地后叠工作点 |
| S-F10 | Hybrid fine-window 演化条带（TGV/Stats 参照 + 5K–60K 序列，per-panel 归一化 + 保真/锐度标注） | `output/paper_figures/figS10_v9a_strip.{png,pdf}`（`scripts/paper_figures/figS10_v9a_strip.py`；源 cache npy + `output/ep10_tgv_sr/best_hr_temperature.npy`） | ✅ CVPR 风格（06-12）；诊断原稿仍在 `output/ep07_v9_review/` |
...
| S-F14 | MAP-TV 基准结构证据：zigzag 三剖面对照 + 四方法 highpass 全景 | `output/paper_figures/figS14{a,b}_*.png`（收编自 `ep15_info_limit/m4_deconv_anchor/`） | ✅ 选编收编（06-12） |
| S-F15 | MTF 频响 + 有效 SNR 可恢复性热图（2x 可行/4x 出界边界） | `output/paper_figures/figS15{a,b}_*.png`（收编自 `ep03_theoretical_limits/`） | ✅ 选编收编（06-12） |
| S-T1 | T1 扩展版（全 selected methods × 全列） | `output/ep11_unified_harness/all_arm_metrics.csv` | ✅ |
...
已完成（CPU/GPU）: F1(Task A) · F2+S-F1(06-12 重绘) · F3 当前稿(Task B) ·
              F5/T1/T2(Task D) · F7 经典方法(Task C) · S-F2/S-F9/S-F10(06-12 重绘) ·
              两个策展 notebook
待做（CPU）: F0 teaser crop · S-F6 负结果组图 · S-F7 对齐 gate 图（素材齐，选图组版） ·
              F3/F3s 按最终 Hybrid+Native-FC / Hybrid+ResObs 文案可选 refresh
可选 GPU 后续: F7 learned/GPU 方法补线（非主文硬门槛）
```
