...
| `configs/` | 物理参数单一来源：stage 标定、PSF 标定、噪声底、对齐结果 | §3.1 标定链 |
| `tcforge/` | 物理匹配合成平台（独立包） | §4.2 |
| `algos/ep06_sr_poc/` | forward 算子 + 指标库 | §3.1 / 附录 A.3 |
| `algos/ep07_unet_sr/` | 学习方法训练/推理/评估代码 + 诊断管线 | §4.3 / §5 |
| `algos/ep10_*/` | drizzle / MAP-TV / TGV 经典方法 | §4.1 |
| `algos/ep15_info_limit/` | FRC、$\sigma$ 仲裁、去卷积基准 | 附录 A.4 / §3.1 |
| `algos/ep16_budget_robustness/` | 帧预算与鲁棒性矩阵 | §5.4–5.5 |
| `scripts/paper_figures/` | 论文图生产脚本 | F1–F7 |
| `paper/reports/`、`research_log/` | 正式报告与全程实验日志 | 可追溯性 |

原始数据（`data/`）和实验产物（`output/`）不入版本控制，前者需手动放置，后者由脚本重建。

### E.1.2 环境配置

本项目使用 UV 进行依赖管理。根目录 venv（`uv sync` + `uv pip install -e core/`）用于全局脚本、notebook 和数据审计。每个算法目录（`algos/xxx/`）是独立项目，拥有自己的 venv 和 lockfile，通过 `uv sync` 创建。共享代码以 `pip install -e ../../core` 方式安装。TGV 经典方法因包含 C/CUDA 编译依赖而使用 conda 环境（`environment.yml`）。各算法环境之间零耦合，可独立运行和删除。
...
| S-F10 Hybrid（V9A）演化条带 | `scripts/paper_figures/figS10_v9a_strip.py` | root |
| D.2 MAP-TV 基准 | `algos/ep15_info_limit/scripts/run_m4_deconv_anchor.py` | ep15（~76 min GPU） |

### E.2.4 训练复现

各变体的完整训练 CLI 存档于 `algos/ep07_unet_sr/scripts/` 下的 `run_training.md`、`run_v9.md`、`run_v10.md`。单个变体 60K 步约需 10–20 小时（取决于 batch size 和 GPU 型号）。
...
诚实声明。 真实数据结论的第三方复现依赖三层证据：协议（本文）、实验日志（`research_log/` 随仓库提供）和中间产物 CSV（`paper/reports/` 随仓库提供）。
